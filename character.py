...
class Fighter:    #int(((self.ad + self.ad + self.ad*0.55) / (100/(100+target.de)))*5)
    ...
    def damageskill(self, target):
        ...
        if self.mp - 50 < 0:
            ...
            a = random.choice(['r', 'l'])
            if a == 'r':
                damm = int(((self.ad + 140 + (target.hp/100)*(1+(self.ad*0.03)) + (self.ad*0.55)) * (100/(100+target.de)))*2)
                target.hp -= damm
                ...
                if b < 90:
                    ...
                    a = int(((self.ad + 50 + (target.hp/100)*(1+(self.ad*0.03))) * (100/(100+target.de)))*4)
                    target.hp -= damm
    ...
    def buffdebuff(self, target):
        ...
        if self.mp - 40 < 0 :
            ...
            self.de *= 1.5
            self.ad *= 2
            slow_print(f'{self.name}이/가 한턴동안 방어력이 0.5배가 증가하고 공격력이 영구적으로 두배로 증가합니다.')
            print()
            self.mp += self.rmp - 80
    ...
    def ultimate(self, target):
        ...
        if self.uturn > 0:
            ...
            damm = int((((self.ad + self.ad + self.ad*0.55) * (100/(100+target.de)))*5)*2)
    ...
    def explanation(self):
        ...
        slow_print('[배면기]는 방어력이 한턴 동안 0.5배가 증가하고 공격력을 영구적으로 2배로 증가시키는 (디)버프 스킬입니다.')
...
class Gambler:
    ...
    def normal(self, target):
        a = random.choice(1, 2000)
        target.hp -= a
        slow_print(f'{self.name}이/가 ??? 을 시도합니다!')
        slow_print(f'{self.name}이/가 {target.name}에게 {a}만큼 피해를 입혔습니다.')
        if target.hp > 0:
            slow_print(f'{target.name}의 체력이 {target.hp} 남았습니다.')
        else:
            slow_print(f'{target.name}이/가 사망하였습니다!')
